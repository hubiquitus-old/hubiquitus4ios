...
#import "HClient.h"
#import "transport/socketio/HSocketIOTransport.h"
#import "transport/xmpp/HXmppTransport.h"
#import "SBJson.h"
#import "HMessage.h"
...
@property (strong, nonatomic) id<HTransport> transport;

- (HMessage*)hmessageFromJSonString:(NSString*)jsonMessage;
...
+ (id)clientWithUsername:(NSString*)username password:(NSString*)password delegate:(id<HClientDelegate>)delegate options:(HOptions*)options {
    ...
    return [[HClient alloc] initWithUsername:username password:password delegate:delegate options:options];
}
...
+ (id)clientWithUsername:(NSString *)username password:(NSString *)password callbackBlock:(void (^)(NSString * context, NSDictionary * data))callback options:(HOptions*)options {
    ...
    return [[HClient alloc] initWithUsername:username password:password callbackBlock:callback options:options];
}
 ...
 */
- (id)initWithUsername:(NSString*)username password:(NSString*)password delegate:(id<HClientDelegate>)aDelegate options:(HOptions*)options {
    
    self = [super init];
    if (self) {
        if (!options) {
            _options = [HOptions optionsWithDict:nil];
        } else {
            _options = options;
        }
        ...
        if ([options.gateway.transport compare:@"xmpp"] == NSOrderedSame) {
            transport = [[HXmpp alloc] initWithOptions:_options delegate:self];
        } else {
            transport = [[HSocketIO alloc] initWithOptions:_options delegate:self];
        }
...
- (void)notifyIncomingMessage:(NSDictionary*)data context:(NSString *)context {
    if (_callback != nil) {
        ...
        if ([context isEqualToString:@"message"]) {
            ...
            HMessage * message = [self hmessageFromJSonString:messageAsStr];
...
- (HMessage *)hmessageFromJSonString:(NSString *)jsonMessage {
    ...
    HMessage * message = [[HMessage alloc] initWithDictionnary:messageAsDict];
